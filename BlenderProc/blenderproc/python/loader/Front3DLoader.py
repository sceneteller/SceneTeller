...
def load_front3d(json_path: str, future_model_path: str, front_3D_texture_path: str, scene_id: str, label_mapping: LabelIdMapping,
    ...
    scene_id = scene_id
    root_path = "/var/scratch/bmocal/data/3D-Front/preprocessed/bedroom"
    ...
    created_objects_nf = _Front3DLoader.create_mesh_objects_from_file(data, front_3D_texture_path,ceiling_light_strength, label_mapping, json_path, scene_id)

    all_loaded_furniture = _Front3DLoader.load_furniture_objs(data, future_model_path,lamp_light_strength, label_mapping, scene_id, json_path, root_path)

    created_objects = _Front3DLoader.move_and_duplicate_furniture(data, all_loaded_furniture, scene_id)

    # add an identifier to the obj
    for obj in created_objects:
        obj.set_cp("is_3d_front", True)

    return created_objects, created_objects_nf[0]
...
class _Front3DLoader:
    ...
    def create_mesh_objects_from_file(data: dict, front_3D_texture_path: str, ceiling_light_strength: float,
                                      label_mapping: LabelIdMapping, json_path: str, scene_id: str) -> List[MeshObject]:
        """
        ...
        for mesh_data in data["mesh"]:
            ...
            if not used_obj_name.lower().startswith("floor"):
                continue
            if used_obj_name == "":
                used_obj_name = "void"
            ...
            for room_id, room in enumerate(data["scene"]["room"]):
                if (scene_id == room['instanceid']):
                    for child in room["children"]:
                            if mesh_data["uid"] == child["ref"]:
    ...
    def load_furniture_objs(data: dict, future_model_path: str, lamp_light_strength: float,
                            label_mapping: LabelIdMapping, scene_id: str, json_path: str, root_path: str) -> List[MeshObject]:
        """
        ...
        :return: The list of loaded mesh objects.
        """
        bbox_id = json_path.split("/")[-1][:-5] + "_" + scene_id
        bbox_path = os.path.join(root_path, bbox_id, "boxes.npz")
        data_bbox = np.load(bbox_path)


        path_stats = os.path.join(root_path, "dataset_stats.txt")
                ...
                if scene_id == room['instanceid']:
                    for child in room["children"]:
                        if "furniture" in child["instanceid"]:
                            if ele["uid"] == child["ref"]:
                                jid = ele["jid"]
                                if jid in data_bbox['jids']:
                                    id_jid = np.where(data_bbox['jids'] == jid)[0][0]
                                    labels = data_bbox['class_labels'][id_jid]
    ...
    @staticmethod
    def move_and_duplicate_furniture(data: dict, all_loaded_furniture: list, scene_id: str) -> List[MeshObject]:
        ...
        for obj in all_loaded_furniture:
            print("{}-{}-{}".format(obj.get_name(), obj.get_cp("uid"), obj.get_cp("jid")))'''
