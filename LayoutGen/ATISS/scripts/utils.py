...
def export_scene(output_directory, trimesh_meshes, tr_floor, obj_list, names=None):
    if names is None:
        names = [
    ...
    for i in range(len(names)):
        names[i] = names[i][:-4] + "-" + obj_list[i] + ".obj"
        mtl_names[i] = mtl_names[i] + "-" + obj_list[i]


    # Write FLOOR
    obj_out = trimesh.exchange.obj.export_obj(tr_floor[0])
    with open(os.path.join(output_directory, "floor.obj"), "w") as f:
        f.write(obj_out)
